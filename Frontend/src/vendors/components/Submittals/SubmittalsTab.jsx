...
import { getVendorBidSubmittals } from '../../services/api';
...
function SubmittalsTab({ bidId, onViewSubmittal, onAddNew, onDelete, refreshTrigger = 0 }) {
  ...
  const fetchSubmittals = async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBidSubmittals(bidId, currentPage, rowsPerPage, searchQuery, statusFilter);
      if (res && res.success) {
        setSubmittals(res.submittals || res.data?.submittals || []);
  ...
  return (
    <div className={styles.logContainer}>
      <div className={styles.logHeader}>
        <h2 className={styles.logTitle}>Submittal Log</h2>
      ...
      <div className={styles.tableWrapper}>
        <table className={styles.rfiTable}>
          ...
          <tbody>
            {loading && submittals.length === 0 ? (
              ...
              <tr><td colSpan="7" style={{ textAlign: 'center', padding: '20px' }}>No submittals found.</td></tr>
            ) : (
              submittals.map((item, idx) => {
                    ...
                    >
                      <td className={styles.srNoCell}>
                        ...
                        {item?.priority === 'High' && (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <WarningIcon />
                            <span>SUB-{item?.id || 7566 + absoluteIndex}</span>
                          </div>
                        </div>
                      </td>
                      <td className={styles.titleCell}>
                        <span className={styles.rfiTitle}>{item?.title || 'Untitled Submittal'}</span>
                      </td>
                      <td className={styles.notesCell} style={{ color: '#475467' }}>{item?.type || 'Shop Drawing'}</td>
                      <td className={styles.notesCell} style={{ color: '#475467' }}>{formatDetailDate(item?.due_date)}</td>
                      ...
                      <td className={styles.actionCell}>
                        <div className={styles.actionIcons} onClick={e => e.stopPropagation()}>
                          <button className={styles.iconBtn} title="View" onClick={() => item?.id && onViewSubmittal?.(item)}>
                            <EyeIcon />
                          </button>
                          <button
                        ...
                        <td colSpan="7" style={{ padding: 0 }}>
                          <div className={styles.historyWrapper}>
                                  ...
                                  <div className={styles.historyRight}>
                                    ...
                                    {entry?.file_url && (
                                      <div className={styles.submittalHistoryPdf}>
                                        ...
                                        <span className={styles.pdfVersion}>Version - {entry?.version || (history.length - entryIdx)}</span>
                                      </div>
                                    )}
                                  </div>
                                </div>
                            ...
                            <div style={{ textAlign: 'center' }}>
                              <button className={styles.viewMoreBtn} onClick={() => item?.id && onViewSubmittal?.(item)}>
                                View More
                              </button>
                            </div>
                          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 20, marginTop: 20, marginBottom: 20 }}>
          <button 
            onClick={() => setCurrentPage(c => c > 1 ? c - 1 : 1)} 
            disabled={currentPage === 1 || submittals.length === 0}
          ...
          <span style={{ fontSize: 14, fontFamily: "'Inter', sans-serif", color: '#344054' }}>
            Page {submittals.length > 0 ? currentPage : 0} of {totalPages}
          </span>
          <button 
            onClick={() => setCurrentPage(c => c < totalPages ? c + 1 : totalPages)} 
            disabled={currentPage === totalPages || submittals.length === 0}
            style={{ padding: '8px 16px', background: '#F2F4F7', border: '1px solid #D0D5DD', borderRadius: 6, cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}
          >
            Next
          </button>
